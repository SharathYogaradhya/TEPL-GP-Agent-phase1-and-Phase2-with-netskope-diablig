...
function Test-GlobalProtectConnected {
    ...
    while ($elapsed -le $MaxWaitSeconds) {
        ...
        if ($serviceRunning) {
            ...
            $gpAdapterUp = [bool]($gpAdapter | Where-Object { $_.Status -eq "Up" })
        }

        if ($serviceRunning -and $gpAdapterUp) {
            Write-Log "GlobalProtect service is running and the virtual adapter is up. Treating GlobalProtect as connected."
            return $true
        }
    ...
    Write-Log "Could not confirm GlobalProtect is connected (service running + virtual adapter up) within $MaxWaitSeconds seconds."
    return $false
}

# Shared Netskope functions (v3): two-step uninstall (plain attempt, then
# password retry only if needed), dot-sourced so they log through this
# script's own Write-Log function.
$netskopeFunctionsPath = "C:\PaloAlto Package\Netskope Script\Netskope-Functions-v3.ps1"
if (-not (Test-Path -Path $netskopeFunctionsPath)) {
    Write-Log "Netskope-Functions-v3.ps1 not found at path: $netskopeFunctionsPath"
    exit 1
}
...
# PLACEHOLDER - replace with the real disable password from the Netskope
# tenant admin console (Settings > Client Configuration > General) before
# relying on this against a tamper-protected endpoint.
