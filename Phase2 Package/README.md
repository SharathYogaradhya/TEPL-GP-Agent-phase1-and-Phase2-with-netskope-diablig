...
Self-contained deployment package for Phase2 (certificates + GlobalProtect + Portal/Prelogon auto-connect config + Netskope uninstall once GlobalProtect is confirmed connected). Uses `TEPL Phase2 Windows-Final-v11.ps1`, the current recommended version.
...
3. Run `Phase2 Script\TEPL Phase2 Windows-Final-v11.ps1` as Administrator.
...
├── Phase2 Script\
│   └── TEPL Phase2 Windows-Final-v11.ps1
└── Netskope Script\
    └── Netskope-Functions-v6.ps1  (dot-sourced by the Phase2 script)
```

This package is a copy of `Phase2 Script/TEPL Phase2 Windows-Final-v11.ps1` and `Netskope Script/Netskope-Functions-v6.ps1` at the repo root — those original files are never modified. If a future version (v12+) becomes the recommended one, update the copy in this package rather than editing v11 in place.
