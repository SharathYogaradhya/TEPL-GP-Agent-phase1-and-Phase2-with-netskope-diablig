...
Self-contained deployment package for Phase2 (certificates + GlobalProtect + Portal/Prelogon auto-connect config + Netskope uninstall once GlobalProtect is confirmed connected). Uses `TEPL Phase2 Windows-Final-v13.ps1`, the current recommended version.
...
3. Run `Phase2 Script\TEPL Phase2 Windows-Final-v13.ps1` as Administrator.
...
After each uninstall attempt, it also polls for up to 90 seconds to confirm Netskope is actually gone (folder + registry entry removed), instead of a single fixed 15-second wait — a real-machine test showed a genuinely successful uninstall could still take longer than 15 seconds to fully clean up, which the older fixed wait misreported as a failure.
...
├── Phase2 Script\
│   └── TEPL Phase2 Windows-Final-v13.ps1
└── Netskope Script\
    └── Netskope-Functions-v7.ps1  (dot-sourced by the Phase2 script)
```

This package is a copy of `Phase2 Script/TEPL Phase2 Windows-Final-v13.ps1` and `Netskope Script/Netskope-Functions-v7.ps1` at the repo root — those original files are never modified. If a future version (v14+) becomes the recommended one, update the copy in this package rather than editing v13 in place.
