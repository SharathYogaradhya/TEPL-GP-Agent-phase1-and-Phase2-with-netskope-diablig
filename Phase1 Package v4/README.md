...
1. Copy this entire `Phase1 Package v4` folder onto the target machine and place/rename it as `C:\PaloAlto Package\` (the script's paths are hardcoded to `C:\PaloAlto Package\...`).
...
All five of these behaviors (success, already-installed skip, MSI failure, service-restart failure, service-never-reaches-Running) were verified by running the real, unmodified function against mocked Windows cmdlets — not just read through.
