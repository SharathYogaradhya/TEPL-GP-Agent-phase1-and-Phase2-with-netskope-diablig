...
# Shared Netskope functions (v3): Uninstall-NetskopeAgent now tries a plain
# uninstall first, and only retries with a password if that attempt does
# not actually remove the client (treated as a tamper-protection block).
...
# relying on this against a tamper-protected endpoint. If left as a dummy
# value on a real tamper-protected machine, the retry step will also fail
# and the script will log that clearly rather than silently succeeding.
