# Shared Netskope client functions (v5).
...
#   . "C:\PaloAlto Package\Netskope Script\Netskope-Functions-v5.ps1"
#
# Changes from v4 (Netskope-Functions-v4.ps1), based on real data collected
# from a machine with Netskope actually installed:
# - The confirmed real deployment uses an MSI installer (UninstallString of
#   the form "MsiExec.exe /I{GUID}"), not the Inno Setup unins000.exe this
#   file previously assumed as a fallback. The install folder itself
...
#   presence check now tests for that folder instead of a specific,
#   installer-type-specific file name.
# - If the registry uninstall entry can't be found AND we can't determine
#   the real uninstall command, Uninstall-NetskopeAgent no longer guesses
#   at a command to run (the old fallback tried to execute a Inno-style
#   .exe path that would not exist for an MSI install). It now logs that
#   an automated uninstall command could not be determined and stops,
#   rather than attempting a command that would just fail.
#
# STILL UNCONFIRMED: the MSI public property (if any) Netskope uses to pass
# a tamper-protection disable password to `msiexec /x`. The MSI branch of
# Invoke-NetskopeUninstaller does not add the password to the command line
# for this reason - confirm the correct property name with Netskope
# support/documentation before adding it here.
...
#
# NOTE: for an MSI-based install, the password is currently NOT added to
# the command line - the correct MSI public property for Netskope's
# tamper-protection bypass is not confirmed yet. Confirm with Netskope
# support/docs, then add it here (e.g. "$arguments += ` PROPNAME=$NetskopeDisablePassword`").
function Invoke-NetskopeUninstaller {
    ...
    if ($UninstallString -match "msiexec") {
        ...
        if (-not [string]::IsNullOrWhiteSpace($NetskopeDisablePassword)) {
            Write-Log "A disable password is configured, but the MSI tamper-protection property name is not yet confirmed for this Netskope deployment - the password will NOT be applied to this uninstall attempt. Confirm the correct msiexec property with Netskope support before relying on a password-based retry for this install type."
        }
...
#         likely tamper-protection block). See the note on
#         Invoke-NetskopeUninstaller: for an MSI install, this retry is
#         currently a no-op vs. step 1 until the MSI password property is
#         confirmed.
...
function Uninstall-NetskopeAgent {
    ...
    try {
        ...
        if ($exitCode2 -eq 0 -and -not (Test-NetskopeInstalled)) {
            ...
            Write-Log "Either the configured password is incorrect, the MSI password property is not yet confirmed for this install type, or removal requires an action from the Netskope admin console. See guidance above."
        }
    } catch {
